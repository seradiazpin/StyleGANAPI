...
def downSampleImage(image, dimension):
    ...
    if sh[2] > dimension:
        ...
        return tf.reduce_mean(tf.reshape(image, [-1, sh[1], sh[2] // factor, factor, sh[2] // factor, factor]),
                              axis=[3, 5])
...
class Projector:
    ...
    def set_network(self, Gs, lpips, minibatch_size=1):
        ...
        if self.uniform_latents:
            ...
            self._dlatents_var = tf.Variable(tf.zeros(
                [self._minibatch_size, self._Gs.components.synthesis.input_shape[1], self._dlatent_avg.shape[2]]),
                name='dlatents_var')
            self._noise_in = tf.placeholder(tf.float32, [], name='noise_in')
            dlatents_noise = tf.random.normal(shape=self._dlatents_var.shape) * self._noise_in
        ...
        if self._lpips is None:
            self._lpips = misc.load_pkl(
                'https://drive.google.com/uc?id=1N2-m9qszOeVC9Tq77WxsLnuWwOedQiD2')  # vgg16_zhang_perceptual.pkl
    ...
    def start(self, target_images):
        ...
        tflib.set_vars({self._target_images_var: target_images, self._dlatents_var: np.tile(self._dlatent_avg,
                                                                                            [self._minibatch_size,
                                                                                             self._dlatents_var.shape[
                                                                                                 1], 1])})
